	...
	<div class="max-w-2xl mx-auto">
		...
		<div class="bg-white border border-gray-200 rounded-lg p-6">
			...
			<div class="flex items-start gap-6">
				...
				<div class="flex-1 grid grid-cols-2 gap-x-12 gap-y-4">
					...
					<div>
						...
						<p class="text-gray-800 font-medium"><?= esc(session()->get('nom') ?? 'Dupont') ?></p>
					</div>
					<!-- Prénom -->
					<div>
						<p class="text-sm text-gray-400">Prénom</p>
						<p class="text-gray-800 font-medium"><?= esc(session()->get('prenom') ?? 'Marie') ?></p>
					</div>
					<!-- Email -->
					<div>
						...
						<p class="text-gray-800 flex items-center gap-2">
							...
							<?= esc(session()->get('email') ?? 'marie.dupont@example.com') ?>
						</p>
					</div>
					<!-- Téléphone -->
					<div>
						...
						<p class="text-gray-800 flex items-center gap-2">
							...
							<?= esc(session()->get('telephone') ?? '+33 6 12 34 56 78') ?>
						</p>
					</div>
					<!-- Newsletter -->
					<div class="col-span-2">
						<p class="text-sm text-gray-400">Newsletter</p>
						<p class="text-gray-800 font-medium"><?= (session()->get('newsletter') ?? true) ? 'Abonné(e)' : 'Non abonné(e)' ?></p>
					</div>
				</div>
			</div>
			...
			<div class="flex gap-4 mt-8">
				<button type="button" id="btn-modifier" class="flex-1 flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-600 text-white font-medium py-3 px-6 rounded-md transition-colors">
					<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
						<path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
					</svg>
					Modifier le compte
				</button>
				<button type="button" id="btn-supprimer" class="flex-1 flex items-center justify-center gap-2 bg-white border-2 border-red-500 text-red-500 hover:bg-red-50 font-medium py-3 px-6 rounded-md transition-colors">
					<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
