...
<div class="container mx-auto px-4 py-8">
	<div class="mb-8" style="margin-top:6rem;">
		...
		<h1 class="text-3xl font-bold text-gray-800">Modifier le pays : <?= esc($country['name']) ?></h1>
	</div>
	...
	<div class="bg-white rounded-lg shadow-md p-8">
		<form action="<?= base_url('admin/countries/edit/' . $country['idCountry']) ?>" method="POST">
			<?= csrf_field() ?>
			...
			<div class="mb-6">
				...
				<input type="text" 
					   id="name" 
					   name="name" 
					   value="<?= old('name', esc($country['name'])) ?>" 
			...
			<div class="mb-6">
				...
				<select id="continent" 
					...
					<option value="europe" <?= old('continent', strtolower($country['continent'] ?? '')) === 'europe' ? 'selected' : '' ?>>Europe</option>
					<option value="asie" <?= old('continent', strtolower($country['continent'] ?? '')) === 'asie' ? 'selected' : '' ?>>Asie</option>
					<option value="afrique" <?= old('continent', strtolower($country['continent'] ?? '')) === 'afrique' ? 'selected' : '' ?>>Afrique</option>
					<option value="amerique" <?= old('continent', strtolower($country['continent'] ?? '')) === 'amerique' ? 'selected' : '' ?>>Amérique</option>
					<option value="oceanie" <?= old('continent', strtolower($country['continent'] ?? '')) === 'oceanie' ? 'selected' : '' ?>>Océanie</option>
				</select>
			</div>
			...
			<div class="mb-6">
				...
				<div class="relative">
					<input type="number" 
						   id="cost" 
						   name="cost" 
						   value="<?= old('cost', esc($country['cost'] ?? '')) ?>" 
						   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500"
						   placeholder="Ex: 150"
						   min="0"
						   step="0.01">
