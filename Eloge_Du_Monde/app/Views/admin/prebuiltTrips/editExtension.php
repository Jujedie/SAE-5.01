...
<?= $this->section('title') ?>Modifier une extension<?= $this->endSection() ?>
...
<div class="container mx-auto px-4 py-8">
	<div class="mb-8" style="margin-top:6rem;">
		...
		<h1 class="text-3xl font-bold text-gray-800 mb-2">Modifier une extension</h1>
		<p class="text-gray-600">Modifier l'extension pour le voyage : <?= esc($prebuiltTrip['title']) ?></p>
	</div>

	<div class="bg-white rounded-lg shadow-md p-8">
		<form action="<?= base_url('admin/prebuiltTrips/extension/edit/' . $extension['idTrip']) ?>" method="POST" enctype="multipart/form-data">
			<?= csrf_field() ?>
				...
				<div class="md:col-span-2">
					...
					<?php if (!empty($extension['attachment'])): ?>
						<p class="mt-2 text-sm text-green-600">
							Pièce jointe actuelle: <a href="<?= base_url('uploads/' . $extension['attachment']) ?>" target="_blank" class="underline">Voir le fichier</a>
						</p>
					<?php endif; ?>
				</div>
			</div>
			...
			<div class="mt-6 p-4 bg-purple-50 border border-purple-200 rounded-lg">
				...
				<p class="mt-2 text-xs text-purple-700">Ces informations sont héritées du voyage principal et ne peuvent pas être modifiées</p>
			</div>

			<div class="mt-8 flex items-center justify-end space-x-4">
				<a href="<?= base_url('admin/prebuiltTrips/view/' . $prebuiltTrip['idTrip']) ?>" 
