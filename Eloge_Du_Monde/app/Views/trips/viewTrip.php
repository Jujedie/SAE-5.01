...
<?= $this->section('title') ?>Voyage<?= $this->endSection() ?>
	...
	<div class="max-w-7xl mx-auto px-4">
		<div class="grid grid-cols-2 md:grid-cols-4 divide-x divide-gray-200">
			<div class="py-6 px-4 text-center">
				<svg class="w-6 h-6 mx-auto custom-amber-text mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
				</svg>
				<p class="text-xs text-gray-500 mb-1">Prix</p>
				<p class="text-lg font-semibold custom-amber-text"><?= esc($trip['amount'] ?? '0') ?> €</p>
			</div>
			<div class="py-6 px-4 text-center">
			...
			<div class="py-6 px-4 text-center">
				...
				<p class="text-lg font-semibold text-gray-700"><?= count($trip['steps'] ?? []) ?> étapes</p>
			</div>
		</div>
	</div>
</section>
			...
			<div class="lg:col-span-2 space-y-8">
				...
				<div class="bg-white rounded-lg shadow-md p-6">
					<h2 class="flex items-center text-xl font-semibold text-gray-800 mb-4">
						<svg class="w-5 h-5 mr-2 custom-amber-text" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				...
				<div class="bg-white rounded-lg shadow-md p-6">
					...
					<div class="relative">
						<!-- Ligne verticale -->
						<div class="absolute left-4 top-0 bottom-0 w-0.5 custom-amber-bg-light"></div>
						
						<div class="space-y-6">
							<?php foreach ($trip['steps'] as $index => $step): ?>
							<div class="relative flex items-start ml-4 pl-8">
								<!-- Point -->
								<div class="absolute left-0 w-4 h-4 custom-amber-bg rounded-full border-4 custom-amber-border-light -translate-x-1/2"></div>
								
								<div class="bg-gray-50 rounded-lg p-4 flex-1">
									<div class="flex justify-between items-start">
										<div>
											<span class="text-xs custom-amber-text font-medium">Étape <?= $index + 1 ?></span>
											...
											<?php endif; ?>
											<?php if (!empty($step['nbDays']) || !empty($step['nbNights'])): ?>
											<p class="text-sm custom-amber-text font-medium">
												<?php if (!empty($step['nbDays'])): ?> <?= esc($step['nbDays']) ?> jours <?php endif; ?>
												<?php if (!empty($step['nbNights'])): ?> <?= esc($step['nbNights']) ?> nuits <?php endif; ?>
											</p>
											<?php endif; ?>
										</div>
										<?php if (!empty($step['cost'])): ?>
										<span class="custom-amber-text font-semibold"><?= esc($step['cost']) ?> €</span>
										<?php endif; ?>
									</div>
								</div>
							</div>
			...
			<div class="space-y-6">
				...
				<div class="bg-white rounded-lg shadow-md p-6 sticky top-4">
					<div class="text-center mb-6">
						...
						<p class="text-3xl font-bold custom-amber-text"><?= esc($trip['amount'] ?? '0') ?> €</p>
						<p class="text-gray-500 text-sm">par personne</p>
					</div>
					
					<a 
						href="<?= site_url('booking/create/' . ($trip['idTrip'] ?? '#')) ?>" 
						class="block w-full custom-amber-bg custom-amber-hover text-white text-center py-3 rounded-lg font-medium transition-colors mb-4"
					>
						Réserver ce voyage
					</a>
					...
					<a 
						...
						class="block w-full custom-amber-border custom-amber-text text-center py-3 rounded-lg font-medium transition-colors"
					>
						Nous contacter
					</a>
				</div>
...
<section class="py-8 px-4 bg-white">
	<div class="max-w-7xl mx-auto text-center">
		<a href="<?= site_url('trips') ?>" class="inline-flex items-center custom-amber-text custom-amber-hover-text font-medium">
			<svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
